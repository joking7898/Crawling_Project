...
for i in range(2, 3):
    url = f'https://www.kyongbuk.co.kr/news/articleList.html?page={i}&box_idxno=&sc_sub_section_code=S2N26&view_type=sm'
    r = requests.get(url)
    ...
    for title in news_titles:
        ...
        if href == '':
            ...
            url2 = 'https://www.kyongbuk.co.kr/'+href
            news_contents = requests.get(url2)
            soup = BeautifulSoup(news_contents.text, 'html.parser')
            title = soup.select('#user-container > div.float-center.max-width-1200 > header > div > div')
            # print("제목 : ", title)
            title = str(title)
            cls1 = clean_title(title)
            print(cls1)
            time = soup.select('#user-container > div.float-center.max-width-1200 > header > section > div > ul > li:nth-child(2)')
