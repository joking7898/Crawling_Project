...
#시간 텍스트 추출 함수
def clean_time(text):
    cleaned_text = re.sub('<div id="wtime">', '', text)
    cleaned_text = re.sub('</div>', '', cleaned_text)
...
for i in range(1, 2):
    url = f'http://www.gwangnam.co.kr/sections.php?section=18&page={i}'
    r = requests.get(url)
    ...
    for title in news_titles:
        ...
        if href == '':
            ...
            curs.execute(sql, ('광남일보', cls1, cls2, cls3))
            conn.commit()
#connection 닫기.
conn.close()
