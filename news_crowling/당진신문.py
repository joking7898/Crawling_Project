...
for i in range(1, 2):
    url = f'https://www.idjnews.kr/news/articleList.html?page={i}&sc_section_code=&sc_sub_section_code=S2N3&sc_serial_code=&sc_area=&sc_level=&sc_article_type=&sc_view_level=&sc_sdate=&sc_edate=&sc_serial_number=&sc_word=&sc_word2=&sc_andor=&sc_order_by=E&view_type=sm'
    r = requests.get(url)
    ...
    for title in news_titles:
        ...
        if href == '':
            ...
            url2 = 'https://www.idjnews.kr'+href
            news_contents = requests.get(url2)
            soup = BeautifulSoup(news_contents.text, 'html.parser')
            title = soup.select('#user-container > div.float-center.max-width-1080 > header > header > div')
            # print("제목 : ", title)
            title = str(title)
            cls1 = clean_title(title)
            print(cls1)
            time = soup.select('#user-container > div.float-center.max-width-1080 > header > section > div.info-text > ul > li:nth-child(2)')
            # print(time)
            time = str(time)
