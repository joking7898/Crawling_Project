...
def clean_time(text):
    ...
    cleaned_text = cleaned_text.replace('[', '').replace(']', '').replace(' ', '').replace('<iclass="fafa-commenting-ofa-fw"></i>댓글', '')
...
for i in range(1, 2):
    url = f'https://www.idaegu.co.kr/news/articleList.html?page={i}&sc_section_code=S1N3&sc_sub_section_code=S2N22&sc_serial_code=&sc_area=&sc_level=&sc_article_type=&sc_view_level=&sc_sdate=&sc_edate=&sc_serial_number=&sc_word=&sc_word2=&sc_andor=&sc_order_by=E&view_type=sm'
    r = requests.get(url)
    ...
    for title in news_titles:
        ...
        if href == '':
            ...
            url2 = 'https://www.idaegu.co.kr'+href
            news_contents = requests.get(url2)
            soup = BeautifulSoup(news_contents.text, 'html.parser')
            title = soup.select('#user-container > div.float-center.max-width-1040 > header > header > div')
            # print("제목 : ", title)
            title = str(title)
            cls1 = clean_title(title)
            print(cls1)
            time = soup.select('#user-container > div.float-center.max-width-1040 > header > section > div > ul > li:nth-child(2)')
            print(time)
            time = str(time)
            cls2 = clean_time(time)#2020. 06. 28 형식으로 출력
            if cls2 == '':
                time = soup.select('#user-container > div.float-center.max-width-1040 > header > section > div > ul > li:nth-child(1)')
                time = str(time)
                cls2 = clean_time(time)  # 2020. 06. 28 형식으로 출력
            print(cls2)
            cls3 = ''
